...
here) - Why might your table differ from Michelson’s? - The table might
differ due to rounding error since the values are off by only a few
points.
...
of error was less than the true error since actual `LIGHTSPEED_VACUUM`
does not fall under his uncertainty range. - Make a quantitative
comparison between Michelson’s uncertainty and his error. - Michelson’s
actual error was -151.542 which is higher than his estimate of 51.
...
df_q2 %>% 
  group_by(Distinctness) %>% 
  summarize(VelSd = sd(VelocityVacuum)) %>% 
  ggplot(aes(x = Distinctness, y = VelSd)) +
  geom_col()
```
...
- There was high variation in Michelson’s measurements around a Temp of
  70˚F.
