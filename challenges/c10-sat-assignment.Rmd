...
-   Based on these results, what can we say about the predictive
    ...
    -   `high_GPA` does have a stronger correlation than `both_SAT` so
        it's the better one between the two. But, it's hard to say if
        either of them are good to use for predicting `univ_GPA`.
...
-   By itself, how well does `both_SAT` predict `univ_GPA`?
    -   It has a noticeable positive correlation but it's probably not
        that good by itself to predict anything. `high_GPA` has a higher
...
-   Is `both_SAT` an effective predictor of `univ_GPA`? What specific
    ...
    -   `both_SAT` does have a positive correlation with `univ_GPA` but
        the analysis suggests that it's not an effective predictor of
