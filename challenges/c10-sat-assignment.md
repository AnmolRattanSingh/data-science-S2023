...
- What information do we have about these students?
  - The data contains information like high school GPAs, SAT scores, and
    college grades.
- What kinds of information *do we not have* about these students?
  - We do not have any other demographic information about the students
    other than their scores and grades.
- Based on these missing variables, what possible effects could be
  present in the data that we would have *no way of detecting*?
  - Information like access to resources, which high school the student
    went to, family income, etc. allows us to only perform a narrow data
    analysis.
...
- To what extent does `both_SAT` look like a normal distribution?
  - `both_SAT` does not completely look like a normal distribution. It
    is slightly right-skewed.
- To what extent does `high_GPA` look like a normal distribution?
  - `high_GPA` is the closet one to a normal distribution but it is also
...
```
    ...
    ## 
    ## data:  pull(df_composite, high_GPA) and pull(df_composite, univ_GPA)
...
```
    ...
    ##  Pearson's product-moment correlation
    ## 
    ## data:  pull(df_composite, both_SAT) and pull(df_composite, univ_GPA)
...
- Which correlations are significantly nonzero?
  - Both of the correlations are significantly nonzero at 0.7795631 and
    0.6846776.
- Which of `high_GPA` and `both_SAT` seems to be more strongly
  correlated with `univ_GPA`?
  - `high_GPA` has a stronger correlation with `univ_GPA`.
- How do the results here compare with the visual you created in q2?
  - It’s hard to see in the scatter plot but the `high_GPA` does seem to
    increase slightly more strongly with the `univ_GPA`.
...
- What is the confidence interval on the coefficient of `both_SAT`? Is
  ...
  - The confidence interval is approximately between 0.0020 and 0.0035.
...
- How do the hypothesis test results compare with the results in q6?
  - This model does perform better but also have zero in it’s confidence
    interval making it less reliable.
...
- Between `both_SAT` and `high_GPA`, which single variable would you
  ...
  - Firstly, when we visually examined the scatterplot between
    **`high_GPA`** and **`univ_GPA`**, we observed a stronger linear
    relationship between the two variables compared to **`both_SAT`**
    and **`univ_GPA`**.
  - Secondly, when we fit a linear regression model with each variable
    separately and compared the accuracy of predictions using validation
    data, the model with **`high_GPA`** had a lower error than the model
    with **`both_SAT`**.
