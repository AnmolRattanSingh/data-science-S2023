...
-   Some columns have very large values(I_0, N_p) and some columns have very small values (L, mu_f).
-   idx, which I initially thought was the row index, only ranges from 1-35. That is less than the total rows.
...
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   `avg_T` is a resultant variable. It is a good predictor since it's part of the equation for `T_norm`.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   model q4 appears to be more accurate. The data points fit the slope of the line better in the case of q4.
...
-   How much do you trust your model? Why?
    -   The model does pretty well. Approx 93% validation values are within the predicted range.
...
-   Are there any other recommendations you would provide?
    -   Even though the model has performed well on the validation data, it's important to continue monitoring its performance on new data to ensure it continues to make accurate predictions. It may be beneficial to include additional variables in the model to improve its accuracy. Consider exploring other variables that may be related to T_norm and including them in the model.
