...
-   To what extent can you tell what shape the distribution of the data has?
    -   There is not enough data to comment on the shape of the distribution.
-   Assuming the scopus is the strength of an individual part made from this aluminum alloy, is the observed variability real or induced?
    -   The observed variability is both real and induced. The real variability is between different parts and the induced variability is from the samples taken from each part.
...
-   Is this estimate of the probability of failure trustworthy? Why or why not?
    -   This is not trustworthy since it considers only 25 samples and also does not have a full distribution.
-   Can you confidently conclude that `POF < 0.03`? Why or why not.
    -   No since the estimate is not trustworthy, we can't conclude that the `POF < 0.03`.
...
-   Assuming your scopus is the probability of failure `POF` defined above, does your estimate exhibit real variability, induced variability, or both?
    -   The estimate exhibits both real and induced variability because it inherits both from observing the distribution of strengths of different parts and from drawing samples from the fitted distribution.
-   Does this confidence interval imply that `POF < 0.03`?
    -   Yes, it does because the confidence interval is fully in the range of POF \< 0.03, with the upper bound at about 0.02.
-   Compare this probability with your estimate from q2; is it more or less trustworthy?
    -   The estimate from the confidence interval is more trustworthy because it accounts for induced variability by giving more "samples" to test, allowing for more accurate estimates of which pieces may or may not fail.
-   Does the confidence interval above account for uncertainty arising from the *Monte Carlo approximation*? Why or why not?
    -   It does not since the answers change if we change the seed or the randomness of the approximation.
-   Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
    -   No, it does not since it uses the same tests to generate the new random tests. Since they are dependent on the physical tests the uncertainty is not accounted for.
-   What could you do to tighten up the confidence interval?
    -   Increasing the sample size of simulated points would tighten the confidence interval.
-   Can you *confidently* conclude that `POF < 0.03`? Why or why not?
    -   One can somewhat conclude that POF \< 0.03 based on the confidence interval but I would need to learn more details about the tests to *confidently* conclude anything. There is unaccounted uncertainty arsing from the limited physical tests which does not allow us to make confident conclusions.
...
-   How does this estimate compare with your Monte Carlo estimate above?
    -   the POF estimate falls within the confidence interval of the Monte Carlo estimate.
...
-   With the scopus as the `POF`, would uncertainty due to *Monte Carlo approximation* be induced or real?
    -   The uncertainty due to Monte Carlo approximation would be induced because it arises from the randomness of the Monte Carlo simulations that are used to estimate the POF.
...
-   With the scopus as the `POF`, would uncertainty due to *limited physical tests* be induced or real?
    -   It can be both since real uncertainty could arise due to material defects and induced uncertainty due to human error.
...
-   Does the confidence interval above account for uncertainty arising from *limited physical tests* (`df_samples`)? Why or why not?
    -   Yes, Bootstrapping does not directly account for limited physical tests. However, it can be used to estimate the uncertainty associated with a limited set of observations, which can help in making decisions about further testing.
-   Can you confidently conclude that `POF < 0.03`? Why or why not?
